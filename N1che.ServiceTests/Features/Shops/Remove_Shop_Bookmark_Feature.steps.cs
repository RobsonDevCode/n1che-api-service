...
public partial class Remove_Shop_Bookmark_Feature : FeatureFixture
{
    private const string SuccessLog = "Shop bookmark removed";
    ...
    private string _bookmarkNotFoundMessage = string.Empty;
    ...
    public Remove_Shop_Bookmark_Feature()
    {
    ...
    private async Task RemoveShopBookmark_Is_Called(Guid id)
    {
        _requestedId = id;
        _scopeValues = new Dictionary<string, object> { ["ShopId"] = id, ["UserId"] = _callerId };
        _endpointLog = $"Removing bookmark of shop {id}";
        _bookmarkNotFoundMessage = $"Bookmark for shop {id} not found";
        _shopNotFoundMessage = $"Shop {id} not found";

        _response = await Client.RemoveShopBookmark(id);
    }
