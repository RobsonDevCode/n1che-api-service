...
public partial class Add_Shop_Vote_Feature : FeatureFixture
{
    private const string SuccessLog = "Shop vote added";
    ...
    private string _duplicateVoteMessage = string.Empty;
    ...
    private async Task AddShopVote_Is_Called(Guid id)
    {
        _requestedId = id;
        _scopeValues = new Dictionary<string, object> { ["ShopId"] = id, ["UserId"] = _callerId };
        _endpointLog = $"Adding vote for shop {id}";
        _duplicateVoteMessage = $"Vote for shop {id} already exists.";
        _shopNotFoundMessage = $"Shop {id} not found";

        _response = await Client.AddShopVote(id);
    }
    ...
    private async Task No_Vote_Is_Stored()
    {
        var votes = await ShopVotesPersistenceProvider.GetByShopId(_requestedId);
